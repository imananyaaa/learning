<aside class="sidebar-bg fixed inset-y-0 left-0 z-50 w-64 flex flex-col transform transition-transform duration-300 md:translate-x-0"
    ...
    <nav class="flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
        ...
        <a href="{{ url('backend/dashboard') }}" class="nav-link {{ request()->routeIs('backend/dashboard') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fa-solid fa-gauge-high"></i></span> Dashboard
        </a>

        <p class="nav-section">Pengelolaan</p>
        <a href="{{ url('backend/fasilitas') }}" class="nav-link {{ request()->routeIs('backend/fasilitas*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fa-solid fa-building"></i></span> Fasilitas
        </a>
        <a href="{{ url('backend/event') }}" class="nav-link {{ request()->routeIs('backend/event*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fa-solid fa-calendar-days"></i></span> Event
        </a>
        <a href="{{ url('backend/ulasan') }}" class="nav-link {{ request()->routeIs('backend/ulasan*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fa-solid fa-star"></i></span> Ulasan

        </a>
        <a href="{{ url('backend/kontak') }}" class="nav-link {{ request()->routeIs('backend/kontak*') ? 'active' : '' }}">
    ...
    <div class="p-4 border-t border-white/10 flex-shrink-0">
        <div class="flex items-center gap-3 px-2 mb-3">
            ...
            <div class="min-w-0">
                <p class="text-white text-sm font-semibold truncate">User</p>
                <p class="text-blue-300/70 text-xs capitalize">User</p>
            </div>
        </div>
